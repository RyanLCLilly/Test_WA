...
var requiredEnvVars = ['CF_CALLBACK_URL', 'CF_CLIENT_ID', 'CF_CLIENT_SECRET', 'SESSION_SECRET', 'AUTH_REQUIRED', 'SF_AUTHORIZE_URL', 'SF_TOKEN_URL'];

var missingVars = [];

// check that all environment variables are present
requiredEnvVars.forEach(function (variable) {
...
if (missingVars.length > 0) {
  print(chalk.bgRed('The following environment variables are missing!\n -- ' + missingVars.join('\n -- ')));
...
var authMiddleware = function authMiddleware(req, res, next) {
  ...
  if (process.env.AUTH_REQUIRED === 'false' || req.session && req.session.isAuthed) {
    ...
    res.redirect('' + process.env.SF_AUTHORIZE_URL + ('?response_type=code&client_id=' + process.env.CF_CLIENT_ID) + ('&redirect_uri=' + encodeURIComponent(process.env.CF_CALLBACK_URL)));
  }
};
...
var authenticate = function authenticate(router) {
  ...
  router.get('/auth/callback', function (req, res) {
    ...
    fetch(process.env.SF_TOKEN_URL + '?code=' + req.query.code + ('&grant_type=authorization_code&client_id=' + process.env.CF_CLIENT_ID) + ('&client_secret=' + process.env.CF_CLIENT_SECRET) + ('&redirect_uri=' + encodeURIComponent(process.env.CF_CALLBACK_URL)), { method: 'POST', body: 'a=1' }).then(function (resp) {
      return resp.json();
    }).then(function (json) {
