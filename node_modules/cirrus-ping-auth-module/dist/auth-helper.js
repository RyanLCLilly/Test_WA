...
var verify = function verify(idToken) {
  return new Promise(function (resolve) {
    ...
    fetch(process.env.OPENID_ISSUER + '/pf/JWKS', {
      method: 'GET',
      body: 'a=1'
    }).then(function (resp) {
      return resp.json();
...
var redirectToAuth = function redirectToAuth(req, res) {
  getAuthCallbackURL(req).then(function (callback) {
    ...
    res.redirect(process.env.OPENID_ISSUER + '/as/authorization.oauth2' + ('?scope=' + process.env.OPENID_SCOPE) + ('&client_id=' + process.env.OPENID_CLIENT_ID) + '&response_type=id_token' + ('&nonce=' + process.env.OPENID_NONCE) + ('&state=' + encodeURIComponent(req.originalUrl)) + ('&redirect_uri=' + encodeURIComponent(callback)));
  }).catch(function (e) {
    res.status(500, {
      error: e
    });
